...
impl Species {
    ...
    pub fn new(first_org: Rc<RefCell<Genome>>, species_id: i32) -> Species {
        let fitness = first_org.borrow().fitness();
        let leader = (*first_org.borrow()).clone();
        Species {
            ...
            age: 0,
            leader: leader,
            members: vec![first_org],
            spawns_rqd: 0.0,
        }
    }

    //这个函数向这个物种添加一个新的成员，并相应地更新成员变量
    pub fn add_member(&mut self, new_member: Rc<RefCell<Genome>>) {
        //新成员的适应分比最好的适应分更好吗？
        let fitness = new_member.borrow().fitness();
        if fitness > self.best_fitness {
            self.best_fitness = fitness;
            self.gens_no_improvement = 0;
            self.leader = (*new_member.borrow()).clone();
        }
    ...
    pub fn adjust_fitnesses(&mut self) {
        // let mut total = 0.0;
        let len = self.members.len();
        for i in 0..self.members.len(){
            let mut fitness = self.members[i].borrow().fitness();
            //如果物种年轻，提高适应分成绩
            ...
            self.members[i].borrow_mut().set_adj_fitness(adjust_fitnesses);
        }
    }
